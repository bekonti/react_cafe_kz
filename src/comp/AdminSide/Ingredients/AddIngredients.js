...
function AddIngredients() {
    ...
    const handleSubmit = event => {
        ...
        addIngredients(inputData);
        setName("");
        event.preventDefault();
    }

    async function addIngredients(inpdd) {
        ...
        const response = axios.post(requests.addingredient, {"name": name},
            {
                headers: {
    ...
    return (
        <div>
            <form action="" onSubmit={handleSubmit}>
                ...
                <button className="btn btn-success">Add Ingredients</button>
            </form>
        </div>
    )
}
